...
The communication between these two software is based on the Legacy remote API of V-REP for Matlab. A class, named "Vrep_STORE"  has been created in Matlab which allow the setting/retrieving of parameters of V-REP. Differents function callback has been created from the V-REP side also. 
...
Based on the two previous matrix, it is possible to create the directed graph of this application (see fig. 6). It exists in total NB(root_nodes)*NB(end_nodes)=18 possibles paths in the graph. A path is represented by a sequence of nodes, which represent themselves the feasible tasks for a specific mobile base position. it also exits null nodes (see nodes on top of fig.6) which coresponds to the empty cells of the reduced matrix of feasible tasks. Among those 18 paths, it exists 5 paths with a length of 2 nodes, and 13 paths with a length of 3 nodes. 
...
- the second task can only be realized with the second and third value of the swivel angle
- and so on ...
